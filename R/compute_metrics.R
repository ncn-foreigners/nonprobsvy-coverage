...
compute_metrics <- function(reps_dt, cell, cfg, runtime) {
  ...
  rows <- lapply(cfg$alpha, function(a) {
    ...
    data.table::data.table(
      ...
      est_method         = as.character(cell$est_method %||% NA),
      gee_h_fun          = as.integer(cell$gee_h_fun %||% NA),
      method_outcome     = as.character(cell$method_outcome %||% NA),
      family_outcome     = as.character(cell$family_outcome %||% NA),
      k                  = as.integer(cell$k %||% NA),
      pmm_match_type     = as.integer(cell$pmm_match_type %||% NA),
      pmm_weights        = as.character(cell$pmm_weights %||% NA),
      pmm_reg_engine     = as.character(cell$pmm_reg_engine %||% NA),
      vars_selection     = as.logical(cell$vars_selection %||% NA),
      penalty            = as.character(cell$penalty %||% NA),
      bias_correction    = as.logical(cell$bias_correction %||% NA),
      target_var         = as.character(cell$target_var %||% NA),
